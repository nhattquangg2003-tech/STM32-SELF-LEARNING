...
void GPIO_Init(void)
{
	// LED PB2
	    ...
	    // thêm chân a10 cho led ngoài
	    GPIO_InitTypeDef gpio_toggle1 = {0};
	    gpio_toggle1.Mode = GPIO_MODE_OUTPUT_PP;
	    gpio_toggle1.Pin = GPIO_PIN_10;
	    gpio_toggle1.Pull = GPIO_NOPULL;
	    gpio_toggle1.Speed = GPIO_SPEED_FREQ_LOW;
	    HAL_GPIO_Init(GPIOA, &gpio_toggle1);
 ...
 void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
 ...
 void TIMER3_Init(void)
 {
		...
		htimer3.Init.Prescaler = 5599;  // 56MHz/5600 = 10kHz
		htimer3.Init.Period = 5000 - 1; // 10kHz/5000 = 2Hz toggle → 1Hz xung
 ...
 {
     if(htim->Instance == TIM3)  // ← check để phân biệt
     {
         HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);

     }

     if(htim->Instance == TIM2)  // nếu TIM2 cũng có update interrupt
     {
         // xử lý TIM2
     }
 }
